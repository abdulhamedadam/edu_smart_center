...
@section('content')
    <div class="container-fluid py-4">
        <div class="page-header mb-4 d-flex justify-content-between align-items-center">
            <div>
                <h1 class="page-title mb-1">تعديل بيانات المدرس</h1>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">تعديل المدرس</li>
                    </ol>
                </nav>
            </div>
        </div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <a href="{{ route('admin.teachers.show', $teacher) }}" class="btn btn-sm btn-light">
                            عرض ملف المدرس
                        </a>
                    </div>
                    <div class="card-body">
                                ...
                                <div class="col-lg-6">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" name="salary_type" class="form-control @error('salary_type') is-invalid @enderror"
                                               value="{{ old('salary_type', $teacher->salary_type) }}">
                                        @error('salary_type')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="number" step="0.01" name="commission_rate"
                                               ...
                                               value="{{ old('commission_rate', $teacher->commission_rate) }}">
                                ...
                                <div class="col-lg-6">
                                    <div class="mb-3">
                                        ...
                                        <select name="subjects[]" class="form-select select2-search @error('subjects') is-invalid @enderror" multiple>
                                            @php
                                                $selectedSubjects = old('subjects', $teacher->subjects->pluck('id')->all());
                                            @endphp
                                            @foreach($subjects as $subject)
                                                <option value="{{ $subject->id }}" @selected(collect($selectedSubjects)->contains($subject->id))>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="groups[]" class="form-select select2-search @error('groups') is-invalid @enderror" multiple>
                                            @php
                                                $selectedGroups = old('groups', $teacher->groups->pluck('id')->all());
                                            @endphp
                                            @foreach($groups as $group)
                                                <option value="{{ $group->id }}" @selected(collect($selectedGroups)->contains($group->id))>
                                                    {{ $group->name }}
                                                </option>
                                            @endforeach
                                        </select>
